        ...
        resetBackground: function(j) {
            if (this.clearBackground) {
                ...
                switch (this.clearingStrategy) {
                    case "all":
                        if (this.fillStyle) {
                            j.fillRect(0, 0, this.canvas.width, this.canvas.height)
                        } else {
                    ...
                    case "eachobject":
                        for (var d = 0, e = this.objects.length, c, b; d < e; d++) {
                            ...
                            switch (c.drawmode) {
                                case "point":
                                    b = (c.outputlinescale ? c.outputlinescale : c.linescale) + 1;
                                    if (this.fillStyle) {
                                        j.fillRect(Floor(c.rminx) - b, Floor(c.rminy) - b, (Ceil(c.rmaxx) - Floor(c.rminx)) + b * 2 + 1, (Ceil(c.rmaxy) - Floor(c.rminy)) + b * 2 + 1)
                                    } else {
                                        j.clearRect(Floor(c.rminx) - b, Floor(c.rminy) - b, (Ceil(c.rmaxx) - Floor(c.rminx)) + b * 2 + 1, (Ceil(c.rmaxy) - Floor(c.rminy)) + b * 2 + 1)
                                    }
                                    break;
                                case "wireframe":
                                    b = (c.outputlinescale ? c.outputlinescale : c.linescale) * 2;
                                    if (this.fillStyle) {
                                        j.fillRect(Floor(c.rminx) - b, Floor(c.rminy) - b, (Ceil(c.rmaxx) - Floor(c.rminx)) + b * 2 + 1, (Ceil(c.rmaxy) - Floor(c.rminy)) + b * 2 + 1)
                                    } else {
                                        j.clearRect(Floor(c.rminx) - b, Floor(c.rminy) - b, (Ceil(c.rmaxx) - Floor(c.rminx)) + b * 2 + 1, (Ceil(c.rmaxy) - Floor(c.rminy)) + b * 2 + 1)
                                ...
                                case "solid":
                                    if (this.fillStyle) {
                                        j.fillRect(Floor(c.rminx) - 1, Floor(c.rminy) - 1, (Ceil(c.rmaxx) - Floor(c.rminx)) + 1, (Ceil(c.rmaxy) - Floor(c.rminy)) + 1)
                                    } else {
                                        j.clearRect(Floor(c.rminx) - 1, Floor(c.rminy) - 1, Ceil(c.rmaxx) - Floor(c.rminx) + 1, Ceil(c.rmaxy) - Floor(c.rminy) + 1)
                                    }
                                    break
                            }
                        }
                    case "combineobjects":
                        ...
                        if (this.fillStyle) {
                            j.fillRect(Floor(k) - a, Floor(h) - a, (Ceil(g) - Floor(k)) + a * 2 + 1, (Ceil(f) - Floor(h)) + a * 2 + 1)
                        } else {
                            j.clearRect(Floor(k) - a, Floor(h) - a, (Ceil(g) - Floor(k)) + a * 2 + 1, (Ceil(f) - Floor(h)) + a * 2 + 1)
                        }
                }
            }
        },
...
(function() {
    ...
    extend(K3D.RequestAnimController, K3D.BaseController, {
        ...
        frame: function() {
            ...
            if (DEBUG && DEBUG.FPS) {
                b.save();
                var d = Math.round(1000 / (g - this.lastFrameStart))
                    , c = Math.round((this.totalfps += d) / (this.framecount++));
                b.fillStyle = "white";
                b.fillRect(4, 4, 128, 10);
                b.fillStyle = "black";
                b.fillText("TPF: " + f + " FPS: " + d + " AVF: " + c, 4, 12);
                b.restore()
            }
    ...
    extend(K3D.K3DObject, K3D.BaseObject, {
        ...
        init: function(v, l, f) {
            ...
            for (var q = 0, a, o, t; q < 256; q++) {
                ...
                this.depthcueColors[q] = "rgb(" + Ceil(a) + "," + Ceil(o) + "," + Ceil(t) + ")"
            }
        ...
        transformToWorld: function() {
            ...
            for (var q = 0, s = b.length, t; q < s; q++) {
                ...
                h = t.y;
                f = t.z;
                b[q].worldnormal = new Vector3D((r[0] * k) + (r[1] * h) + (r[2] * f),(o[0] * k) + (o[1] * h) + (o[2] * f),(m[0] * k) + (m[1] * h) + (m[2] * f))
            }
        },
    ...
    extend(K3D.SolidRenderer, K3D.Renderer, {
        sortByDistance: function b(g) {
            if (g.sortmode === "sorted") {
                this.quickSortObject(g.worldcoords, g.faces, 0, g.faces.length - 1)
            }
        },
                        ...
                        case "lightsource":
                            if (N.length === 0) {
                                ...
                                for (var F = 0, E = N.length, o, h; F < E; F++) {
                                    o = N[F];
                                    I = PI - o.worldvector.thetaTo2(w.worldnormal);
                                    h = I * ((1 / o.worldvector.distance(w.worldnormal)) * o.intensity) / PI;
                                    A += (h * o.color[0]);
                                    H += (h * o.color[1]);
                                    K += (h * o.color[2])
                                }
                                ...
                                J = w.color;
                                var k = Ceil(A * J[0]) + "," + Ceil(H * J[1]) + "," + Ceil(K * J[2]);
                                if (w.texture === null ) {
                                    t = "rgb(" + k + ")"
                                } else {
                                    t = "rgba(" + k + "," + (1 - (A + H + K) * 0.33333) + ")"
                                }
                                this.renderPolygon(D, y, w, t)
                            }
                            break
        ...
        renderPolygon: function d(t, l, o, s) {
            ...
            if (o.texture === null ) {
                ...
                var k = function(G, D, L, C, J, A, H) {
                        ...
                        var Q = -(L * (M - N) - J * M + H * N + (J - H) * O) * u
                            , P = (J * v + L * (w - v) - H * w + (H - J) * x) * u
                            , z = (D * (M - N) - C * M + A * N + (C - A) * O) * u
                            , y = -(C * v + D * (w - v) - A * w + (A - C) * x) * u
                            , F = (D * (H * N - J * M) + L * (C * M - A * N) + (A * J - C * H) * O) * u
                            , E = (D * (H * w - J * v) + L * (C * v - A * w) + (A * J - C * H) * x) * u;
                    ...
                    ;
                k.call(this, n.slice(0, 3), 0, 0, q.width, 0, q.width, q.height);
                if (n.length === 4) {
                    var r = [];
                    r.push(n[2]);
                    r.push(n[3]);
        ...
        inflatePolygon: function e(p, r) {
            ...
            for (var n = 0, m = p.length, k; n < m; n++) {
                if (n === 0) {
                    k = this.intersection(s[(m - 1) * 2], s[(m - 1) * 2 + 1], s[0], s[1])
                } else {
                    k = this.intersection(s[(n - 1) * 2], s[(n - 1) * 2 + 1], s[n * 2], s[n * 2 + 1])
                }
